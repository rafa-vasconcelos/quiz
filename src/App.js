...
export default function App() {
    ...
    const [score, setScore] = useState({0: 'wrong-answer', 
    1: 'wrong-answer', 
    2: 'wrong-answer', 
    3: 'wrong-answer', 
    4: 'wrong-answer'})
        ...
        useEffect(() => {
            ...
            .then(data => setQuizData([
                data.results[0], data.results[1], data.results[2], data.results[3], data.results[4]
              ]
              ))
            setScore({0: 'wrong-answer', 
            1: 'wrong-answer', 
            2: 'wrong-answer', 
            3: 'wrong-answer', 
            4: 'wrong-answer'})
    ...
    return(
        ...
        <h4>{checkAnswers && `You scored ${scoreCounter.filter(
          (elem) => elem !== 'wrong-answer').length}/${quizData.length} correct answers`}</h4>
        <button onClick={clickCheckAnswers}>{checkAnswers ?
         `Play again` :
